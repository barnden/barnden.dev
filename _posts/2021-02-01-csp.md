...
categories: "web"
excerpt: The Content-Security-Policy (CSP) HTTP header tells the browser which sources it can trust to load content from.
---

{% archive %}
&nbsp;

Most of the article is now ***irrelevant*** due to the switch over to $$\KaTeX$$,<br>but it might still be an interesting read.
{% endarchive %}
...
Option 1 obviously defeats the purpose of using CSP.

For option 2, I'm not sure if the browser will actually use the CSP `meta` tag if it is generated after page load. If the browser does obey the CSP in the delayed `meta` tag, there will be a race condition, as it will take time to generate the hashes, a malicious script could load before all the hashes are loaded.
...
- Attempt to use conditional blocks in nginx configuration
